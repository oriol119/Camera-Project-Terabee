...
while not done and cv2.getWindowProperty("Filtered", cv2.WND_PROP_FULLSCREEN) != -1:
   ...
   # sleep(0.01)
    ...
    a= format(frame_data[2440])
    b= format(frame_data[0])
    c= format(frame_data[80])
    d= format(frame_data[4720])
    e= format(frame_data[4799])
    ...
    rgb_frame[np.where(depth_array >= 770) ] = [0, 250, 0]
